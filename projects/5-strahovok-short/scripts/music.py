...
SR=48000; BPM=88.0; BEAT=60/BPM; BAR=4*BEAT; DUR=118.2
...
for b in range(bars):
    ...
    pr = prog if (b//8)%2==0 else [prog[2],prog[3],prog[0],prog[1]]
    root,notes=pr[b%4]
...
for i in range(int(DUR/BEAT)):
    ...
    tk=np.arange(L)/SR
    bar_i=int((ip/SR)/BAR)
    g=0.35 if (bar_i//8)%3==0 else 1.0
    kck[ip:ip+L]+=np.sin(2*np.pi*(52+70*np.exp(-tk*28))*tk)*np.exp(-tk*9)*0.9*g
...
w=wave.open(SP+"/v2/work/music.wav","w"); w.setnchannels(2); w.setsampwidth(2); w.setframerate(SR)
