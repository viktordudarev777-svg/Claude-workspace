...
dur=len(a)/16000; WIN=10.0; HOP=8.0; segs=[]; t=0.0
while t<dur:
    e=min(t+WIN,dur)
    ...
    print(f"--- {t:.1f}-{e:.1f}: {res.text}",flush=True)
    if e>=dur: break
    t+=HOP
json.dump(segs,open(SP+"/v2/work/zf_windows.json","w"),ensure_ascii=False,indent=1)
