...
import { matchWholeFood, loadWholeFoods, toProductInfo } from '../src/domain/wholeFoods';
import { analyzeProduct } from '../src/domain/analyze';

describe('matchWholeFood', () => {
  it('matches vision labels in English and Russian', () => {
    expect(matchWholeFood([{ name: 'Apple', score: 0.97 }])?.food.id).toBe('apple');
    expect(matchWholeFood([{ name: 'банан', score: 0.9 }])?.food.id).toBe('banana');
    expect(matchWholeFood([{ name: 'French fries', score: 0.88 }])?.food.id).toBe('french-fries');
  });
  ...
  it('returns nothing rather than guessing on unrelated labels', () => {
    expect(matchWholeFood([{ name: 'Bicycle', score: 0.9 }])).toBeNull();
  });

  it('never returns a confidence above the model\'s own', () => {
    const match = matchWholeFood([{ name: 'Apple', score: 0.4 }])!;
...
describe('whole foods through the analyser', () => {
  it('rates a photographed apple green with no additives', () => {
    const apple = loadWholeFoods().find((f) => f.id === 'apple')!;
  ...
  it('rates photographed fries worse than an apple', () => {
    const fries = loadWholeFoods().find((f) => f.id === 'french-fries')!;
    const apple = loadWholeFoods().find((f) => f.id === 'apple')!;
    const score = (food: typeof fries) =>
      analyzeProduct({
        source: 'photo', locale: 'ru', product: toProductInfo(food, 'ru'), nutriments: food.nutriments,
  ...
  it('has usable data for every food in the file', () => {
    for (const food of loadWholeFoods()) {
