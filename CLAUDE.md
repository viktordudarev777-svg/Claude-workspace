...
Diese Agenten sind unter `.claude/agents/*.md` definiert und können namentlich aufgerufen werden (z. B. "lass Karpov das recherchieren" oder "frag Martin"):
...
| **konrad** | Qualitätsmanager — unabhängige, kritische Kontrolle von Karpov/Julian/Marco vor Freigabe, prüft nichts ungeprüft und vergibt Qualitätsstufen |

## Typischer Arbeitsablauf

Standardreihenfolge, sofern nicht anders angewiesen: **Karpov** recherchiert und belegt Fakten → der fachlich zuständige Spezialist (**Artöm**, **Martin** oder **Matthias**) bewertet den Sachverhalt in seinem Gebiet → **Julian** entwickelt daraus Konzept und Design → **Marco** setzt es technisch/gestalterisch um → **Konrad** prüft das Gesamtergebnis unabhängig auf Qualität → **Fedor** prüft das fertige Ergebnis rechtlich, bevor es veröffentlicht wird. Matthias wird nur nach ausdrücklichem Auftrag hinzugezogen, nicht automatisch.
