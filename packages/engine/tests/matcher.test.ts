...
import type { AdditiveDatabase } from '../src/additives/database';
import { findAdditives } from '../src/additives/matcher';
// The engine is pure, so the tests supply the data the same way the app does.
import { loadAdditiveDatabase } from '@foodlens/data';

let db: AdditiveDatabase;
beforeAll(() => {
  db = loadAdditiveDatabase();
});
