...
import { normalize, editDistance } from './text';
 ...
 *
 * Vision APIs return a ranked list, so we walk it in order and take the first
 * label that resolves: the model's own ordering is more reliable than any
 * re-ranking we could do here.
 */
export function matchWholeFood(
  labels: Array<{ name: string; score: number }>,
  foods: readonly WholeFood[],
): WholeFoodMatch | null {
