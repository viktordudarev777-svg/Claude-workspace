...
import type { AdditiveDatabase } from './additives/database';
...
export interface AnalyzeInput {
  ...
  locale: Locale;
  /** The indexed additive data; injected so the engine needs no file access. */
  ...
  /** Injected so the engine stays free of Node built-ins. */
  idFactory?: () => string;
}
...
/** Fallback id generator for runtimes without crypto.randomUUID. */
 ...
 *
 * Pure and synchronous, with no file, network or Node-specific dependency:
 * everything I/O-bound (barcode lookup, OCR, finding alternatives) happens in
 * the caller. That is what lets the same code run on the server and, for
 * offline analysis, inside the mobile app.
 */
export function analyzeProduct(input: AnalyzeInput): AnalyzeOutput {
  const { locale } = input;
  const db = input.additives;
